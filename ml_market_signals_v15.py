...
Cópia estabilizada do V11.11.14 em arquivo próprio do branch principal para
que o boot resiliente não dependa de uma referência histórica durante o deploy.
"""
import time
from concurrent.futures import ThreadPoolExecutor, as_completed

_V115_HIGHLIGHT_CACHE = {}
_V115_HIGHLIGHT_TTL = 900
_V115_MAX_WORKERS = 5
_V115_STATS = {"highlight_requests": 0, "highlight_ok": 0, "highlight_404": 0, "highlight_errors": 0}
...
def _v115_cache_get(key):
    entry = _V115_HIGHLIGHT_CACHE.get(key)
    if not entry:
        return None
    if time.time() - entry[0] > _V115_HIGHLIGHT_TTL:
        _V115_HIGHLIGHT_CACHE.pop(key, None)
        return None
    return entry[1]


def _v115_highlight(pid, token):
    pid = str(pid or "").upper().strip()
    if not pid or not token:
        return None, None
    cached = _v115_cache_get(pid)
    ...
    try:
        ...
        _V115_STATS["highlight_requests"] += 1
        if isinstance(data, dict) and 200 <= status < 300:
            result = (data.get("position"), data)
            _V115_HIGHLIGHT_CACHE[pid] = (time.time(), result)
            _V115_STATS["highlight_ok"] += 1
            return result
        if status == 404:
            _V115_STATS["highlight_404"] += 1
        else:
            _V115_STATS["highlight_errors"] += 1
    except Exception:
        _V115_STATS["highlight_requests"] += 1
        _V115_STATS["highlight_errors"] += 1
    _V115_HIGHLIGHT_CACHE[pid] = (time.time(), (None, None))
    return None, None


def _v115_winner_signals(product, item):
    ...
    available = _v115_num(winner.get("available_quantity"))
    price = _v115_num(winner.get("price"))
    original_price = _v115_num(winner.get("original_price"))
...
def _v115_rescore(item):
    base = _v115_num(item.get("opportunity_score"))
    if base is None:
        return None
    position = _v115_num(item.get("best_seller_position"))
...
def _v115_market_enrich(items):
    global _V115_STATS
    _V115_STATS = {key: 0 for key in _V115_STATS}
    ...
    if pending and token:
        with ThreadPoolExecutor(max_workers=min(_V115_MAX_WORKERS, len(pending))) as pool:
            futures = {pool.submit(_v115_highlight, pid, token): pid for pid in pending}
    ...
    for item in enriched:
        ...
        item.update(_v115_winner_signals(product, item))
        item["market_signal_version"] = "V12.5"
        item["opportunity_score"] = _v115_rescore(item)
    print(
        "[V12.5] sinais de mercado: "
        f"highlights_requests={_V115_STATS['highlight_requests']} "
        f"ok={_V115_STATS['highlight_ok']} "
        f"not_ranked={_V115_STATS['highlight_404']} "
        f"errors={_V115_STATS['highlight_errors']} workers={_V115_MAX_WORKERS}",
        flush=True,
    )
    return enriched


_v119_enrich_ml = _v115_market_enrich
print("[V12.5] sinais de mercado estabilizados ativos | best seller + buy box + pressão competitiva", flush=True)
