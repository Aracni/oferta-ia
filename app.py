...
def shopee_opportunities(payload: dict):
    ...
    try:
        ...
        return {"status":"ok", "engine":"OFERTA IA V10.4", "marketplace":"shopee", "items":selected, "opportunities":selected, "returned":len(selected), "candidates_found":len(items), "message":f"Shopee: {len(selected)} oportunidade(s) analisada(s)."}
    except Exception as exc:
...
def opportunities_central(payload: dict):
    ...
    MODO RÁPIDO (padrão):
      - consulta a Shopee e responde assim que ela terminar;
      - NÃO consulta Mercado Livre, evitando que uma integração lenta
        segure o resultado por minutos.

    MODO COMPLETO:
      - envie include_meli=true para também consultar Mercado Livre.
      - Nesse modo a resposta continua aguardando os dois providers.

    A separação mantém o Mercado Livre disponível sem transformar uma API
    lenta no gargalo do garimpo principal.
    """
    niche = (payload.get("niche") or "").strip()
    limit = max(5, min(20, int(payload.get("limit") or 10)))
    include_meli = bool(payload.get("include_meli", False))
    started = time.time()

    all_items = []
    diagnostics = []

    # Shopee é o provider rápido do modo padrão.
    if not _shopee_credentials_ready():
        diagnostics.append("Shopee não configurada")
    else:
        try:
            items = _shopee_search_products(
                niche or None,
                min(50, max(limit * 2, 20))
            )
            all_items.extend(items)
            diagnostics.append(f"Shopee: {len(items)} candidatos")
        except Exception as exc:
            diagnostics.append(f"Shopee indisponível: {str(exc)[:180]}")

    # Mercado Livre só entra quando explicitamente solicitado.
    if include_meli:
        try:
            ...
            if not token:
                ...
                try:
                    ml = mercadolivre_opportunities(
                        {"niche": niche, "limit": limit}
                    )
                    items = ml.get("items") or ml.get("opportunities") or []
                    all_items.extend(items)
                    diagnostics.append(f"Mercado Livre: {len(items)} candidatos")
                except Exception as exc:
                    diagnostics.append(
                        f"Mercado Livre indisponível: {str(exc)[:180]}"
                    )
        except Exception as exc:
            diagnostics.append(
                f"Mercado Livre não consultado: {str(exc)[:180]}"
            )
    else:
        diagnostics.append("Mercado Livre: não consultado (modo rápido)")

    unique = {}
    for item in all_items:
        key = (
            f"{item.get('marketplace')}:"
            f"{item.get('item_id') or item.get('url') or item.get('name')}"
        )
        if (
            key not in unique
            or float(item.get("opportunity_score") or 0)
    ...
    for item in selected:
        if item.get("competition_estimated"):
            item["competition_note"] = (
                "Pressão competitiva estimada por vendas, comissão e ranking; "
                "não é quantidade de afiliados."
            )

    elapsed_ms = round((time.time() - started) * 1000)

    mode = "completo" if include_meli else "rapido"
    return {
        "status": "ok",
        "engine": "OFERTA IA V10.4",
        "mode": mode,
