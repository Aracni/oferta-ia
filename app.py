...
def _v11_normalize_offer(product_id, detail, offer, rank_position=None, query=""):
    ...
    return {
        "item_id": str(iid), "name": title, "url": url,
        "current_price": price, "old_price": old,
        ...
        "image_url": image, "seller_id": offer.get("seller_id") or seller.get("id"),
        "store": str(seller.get("nickname") or offer.get("seller_nickname") or "Mercado Livre"),
        "marketplace": "mercadolivre", "catalog_product_id": str(product_id),
        "product_id": str(product_id), "rating": offer.get("rating"),
        "condition": offer.get("condition") or offer.get("item_condition") or "new",
        "rank_position": rank_position, "discovery_query": query, "data_confidence": "alta",
    }
...
def _v11_catalog_to_item(token, product_id, rank_position=None, query=""):
    ...
    if isinstance(winner, dict):
        ...
        if p:
            ...
            return p
    items_data, items_status = _v11_fetch_json(token, f"https://api.mercadolibre.com/products/{pid}/items", {"limit":50}, timeout=10)
...
def _v11_enrich_product_payload(pid, data, token, trace=None):
    ...
    if isinstance(winner, dict) and winner_id and winner_price is None and isinstance(price_range, dict):
        ...
        if minimum_price is not None and minimum_price > 0:
            enriched = dict(data); ew = dict(winner); ew["price"] = minimum_price
            enriched["buy_box_winner"] = ew
            return enriched
    if winner_id and winner_price is not None:
        return data
    try:
        items_data, items_status = _V11_ORIGINAL_FETCH_JSON(token, f"https://api.mercadolibre.com/products/{pid}/items", {"limit":50}, 12, trace, "CATALOG")
        if items_status and 200 <= int(items_status) < 300:
            _v11_remember_items(pid, items_data)
            best = _v11_best_catalog_row(pid, items_data)
            if best:
                enriched = dict(data); enriched["buy_box_winner"] = best
                return enriched
    except Exception as exc:
