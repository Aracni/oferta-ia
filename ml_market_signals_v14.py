"""OFERTA IA V11.11.14 — sinais de mercado do Mercado Livre.

Complementa o enriquecimento existente sem inventar comissão, avaliações ou vendas.
Usa dados oficiais já disponíveis no /products/{id} e o ranking /highlights.
"""
import time
from concurrent.futures import ThreadPoolExecutor, as_completed

_V114_HIGHLIGHT_CACHE = {}
_V114_HIGHLIGHT_TTL = 900
_V114_MAX_WORKERS = 5
_V114_STATS = {"highlight_requests": 0, "highlight_ok": 0, "highlight_404": 0, "highlight_errors": 0}
...
def _v114_cache_get(key):
    entry = _V114_HIGHLIGHT_CACHE.get(key)
    if not entry:
        return None
    if time.time() - entry[0] > _V114_HIGHLIGHT_TTL:
        _V114_HIGHLIGHT_CACHE.pop(key, None)
        return None
    return entry[1]


def _v114_highlight(pid, token):
    pid = str(pid or "").upper().strip()
    if not pid or not token:
        return None, None
    cached = _v114_cache_get(pid)
    ...
    try:
        data, status = _v11_fetch_json(
            ...
            timeout=5,
            stage="MARKET_SIGNALS_V11_11_14",
        )
        status = int(status or 0)
        _V114_STATS["highlight_requests"] += 1
        if isinstance(data, dict) and 200 <= status < 300:
            result = (data.get("position"), data)
            _V114_HIGHLIGHT_CACHE[pid] = (time.time(), result)
            _V114_STATS["highlight_ok"] += 1
            return result
        if status == 404:
            _V114_STATS["highlight_404"] += 1
        else:
            _V114_STATS["highlight_errors"] += 1
    except Exception:
        _V114_STATS["highlight_requests"] += 1
        _V114_STATS["highlight_errors"] += 1
    _V114_HIGHLIGHT_CACHE[pid] = (time.time(), (None, None))
    return None, None


def _v114_winner_signals(product, item):
    ...
    available = _v114_num(winner.get("available_quantity"))
    price = _v114_num(winner.get("price"))
    original_price = _v114_num(winner.get("original_price"))
...
def _v114_rescore(item):
    base = _v114_num(item.get("opportunity_score"))
    if base is None:
        return None
    # Best-seller position is a demand confirmation, not a commission proxy.
    position = _v114_num(item.get("best_seller_position"))
    if position is not None and 1 <= position <= 20:
        bonus = 8.0 * (21.0 - position) / 20.0
        base += bonus
    # Being the current catalog winner is a small positive signal: it confirms
    # the publication is already positioned to receive catalog-page sales.
...
def _v114_market_enrich(items):
    global _V114_STATS
    _V114_STATS = {key: 0 for key in _V114_STATS}
    ...
    if pending and token:
        with ThreadPoolExecutor(max_workers=min(_V114_MAX_WORKERS, len(pending))) as pool:
            futures = {pool.submit(_v114_highlight, pid, token): pid for pid in pending}
    ...
    for item in enriched:
        ...
        signals = _v114_winner_signals(product, item)
        item.update(signals)
        item["market_signal_version"] = "V11.11.14"
        item["opportunity_score"] = _v114_rescore(item)

    print(
        "[V11.11.14] sinais de mercado: "
        f"highlights_requests={_V114_STATS['highlight_requests']} "
        f"ok={_V114_STATS['highlight_ok']} "
        f"not_ranked={_V114_STATS['highlight_404']} "
        f"errors={_V114_STATS['highlight_errors']} workers={_V114_MAX_WORKERS}",
        flush=True,
    )
    return enriched


_v119_enrich_ml = _v114_market_enrich
print("[V11.11.14] sinais de mercado ativos | best seller + buy box + pressão competitiva", flush=True)
